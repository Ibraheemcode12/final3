...
public class Comments_likes_Controller {
    ...
    @PostMapping("/like")
    public ResponseEntity<Integer> create_like_for_posts(@RequestBody Like like,
            @RequestHeader("Username") String username) { // the Json obj sent from the user will contain his/her
                                                          // username and the postid they liked
        return ResponseEntity.ok(commentservice.save_like_for_posts(like, username));
    }

    @PostMapping("/like_comment")
    public ResponseEntity<Integer> create_like_for_comment(@RequestBody commentlike like,
            @RequestHeader("Username") String username) { // the Json obj sent from the user will contain his/her
                                                          // username and the postid they liked
    ...
    @PutMapping("/comment")
    public ResponseEntity<Comments> putMethodName(@RequestBody Comments entity,
    ...
    public ResponseEntity<Boolean> deletePosts_likes_comments(@RequestBody deleteobj delete) {
        return ResponseEntity
                .ok(commentservice.Delete_likes_comments_for_posts(delete.getDeletelist(), delete.getUsername()));
    }
    ...
    // the difference between these two is one is made to delete one post and one to
    // delte all posts for one user

    @DeleteMapping("/comment/{deleteid}")
    public ResponseEntity<Boolean> deleteComment(@PathVariable Long deleteid,
            @RequestHeader("Username") String username) {
