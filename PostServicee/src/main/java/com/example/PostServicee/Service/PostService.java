...
public class PostService {
    ...
    // NOTE : THESE RANDOM CHARECHTERS YOU SEE LIKE \u001B[31 m ARE JUST USED TO
    // CHANGE THE STRING COLOR IN THE CONSOLE WHICH MAKES IT EASIER TO NOTICE THE
    // ERROR or WARNING
    // and differentiate between the two.

    // NOTE : THE USERNAME PARMETER IN EACH METHOD IS SENT THROUGH A USERNAME HEADER
    // BY THE API GATEWAY, THE API GATEWAY GETS THE REQUEST FIRST THEN CHECKS
    // FOR THE TOKEN HEADER, VALIDATES THE TOKEN THEN IT GETS THE USERNAME FROM IT,
    // THIS WAY NO ONE WILL BE ABLE TO DO SOMETHING
    ...
    // like a 1000 or more times to load
    // one user does not sound lika a good idea right? We are talking about best
    // senario here what if other users have 2000+ posts?
    // so instead on the front end side we will let the user chose what post he
    // wants and the system will load the comments and likes of that one.
    ...
    public postdto get_one_post(Long id) {
        ...
        try {
            ...
            if (post.isEmpty()) {
                logger.warn("\u001B[33m  NO POST WITH THE ID " + id + " WAS FOUND \u001B[0m"); // Checking if the post
                                                                                               // exists and throwing an
                                                                                               // exception if it does
                                                                                               // not.
                throw new PostNotFoundException("No post with id " + id);
            }

            like_comeents_dto dto = like_comment_feign.getAllComments_likes(id).getBody(); // Using feign to get the
                                                                                           // comments and likes of the
                                                                                           // post after checking that
                                                                                           // it exists

            if (dto.getComments().isEmpty()) {
                logger.info("\u001B[33m  POST WITH THE ID " + id + " does not have any comments yet. \u001B[0m"); // Setting
                                                                                                                  // a
                                                                                                                  // warning
                                                                                                                  // for
                                                                                                                  // debugging
            ...
            return new postdto(post.get(), dto.getLikes(), dto.getComments()); // Returning a post DTO (Data transfer
                                                                               // Object)
                                                                               // that contains the requested post along
                                                                               // with its
                                                                               // Comments and likes
        } catch (Exception error) {
            logger.error("\u001B[31m " + error + " \u001B[0m");
    ...
    public boolean Delete_post(Long id, String username) {
        ...
        try {
            ...
            if (!op.get().getAuthorName().equals(username)) { // Authorizing the request by checking if the authorname
                                                              // of the post matches the username sent through the
                                                              // header, the
                                                              // username value in the header was given by the api gate
                                                              // way after it has validated the token and extracted the
                                                              // username after.
            ...
            like_comment_feign.deletePost(id); // deleting likes and comments of this post by sending a deletetion req
                                               // to the like_comment service.
            return true;
        } catch (Exception error) {
            logger.error("\u001B[31m " + error + " \u001B[0m");
            return false;
    ...
    public Post update_post(Post upost, String username) {
        ...
        try {
            ...
            if (post.isEmpty()) { // Checking if the post exists and throwing and exception if it does not, IF a
                                  // post does not exist this means the api was approached by a 3rd party.
    ...
    @Transactional
    public boolean Delete_posts(String username) { // this main thing this method does is it deletes all the posts for a
                                                   // user when he/she delete their account

        try {

            List<Long> list = postrepo.findAllByauthorName(username).stream().map(p -> p.getId()).toList();

            like_comment_feign.deletePosts(new deleteobj(list, username)); // Sending a list of post ids to the
                                                                           // comment_like service to delete all the
                                                                           // likes and comments reltaed to
            // the users posts.
